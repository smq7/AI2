...
class Generator:
    ...
    def load_maze(self, rows, cols):
        self.grid = np.zeros((rows, cols))
        with open("./Game_data/Map2.txt", "r") as file:
            for y_index, line in enumerate(file):
                for x_index, elem in enumerate(line):
                    if elem == 'W':
                        self.grid[y_index, x_index] = WALL
                    elif elem == "C":
                        self.grid[y_index, x_index] = PASSAGE
    ...
    def fill_grid_with_game_objects(self):
        ...
        # self.grid[(4, 9)] = COIN
        position = random.choice(empty_positions)
        self.grid[position] = DEFAULT_GHOST
        # position = random.choice(empty_positions)
